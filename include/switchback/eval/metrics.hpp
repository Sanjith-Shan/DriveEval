#ifndef SWITCHBACK_EVAL_METRICS_HPP
#define SWITCHBACK_EVAL_METRICS_HPP
...
struct ScenarioMetrics {
  ...
  // the field reaches SQL as NULL. Only sb_bench and the test binary replace
...
// CSV header and one row, matching python/switchback/db/schema.sql exactly.
std::string metricsCsvHeader();
std::string metricsCsvRow(const std::string& run_id, const ScenarioMetrics& m);

}  // namespace sb::eval

#endif  // SWITCHBACK_EVAL_METRICS_HPP
